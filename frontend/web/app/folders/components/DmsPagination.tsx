...
import { getPageNumbers } from '@/app/folders/components/dmsUtils';
import { DMS_PAGE_SIZE_OPTIONS } from '@/app/folders/components/types';

export interface DmsPaginationProps {
    currentPage: number;
    totalPages: number;
    pageSize: number;
    totalItems: number;
...
export default function DmsPagination({
    currentPage,
    totalPages,
    pageSize,
    totalItems,
    startIndex,
    endIndex,
    pageSizeOptions = DMS_PAGE_SIZE_OPTIONS,
    onPageChange,
    onPageSizeChange,
    disabled = false,
}: DmsPaginationProps) {
        ...
        >
            ...
            <div className="flex flex-wrap items-center gap-3 text-xs sm:text-sm text-cu-text-secondary">
                <div className="flex items-center gap-2">
                    <label htmlFor="dms-page-size-select" className="text-xs font-medium text-cu-text-secondary whitespace-nowrap">
                        Rows per page:
                    </label>
                    <select
                        id="dms-page-size-select"
                        value={pageSize}
                        disabled={disabled}
                ...
                <div className="text-xs font-medium">
                    ...
                    of <span className="font-bold text-cu-text-primary">{totalItems}</span> documents
                </div>
            </div>
