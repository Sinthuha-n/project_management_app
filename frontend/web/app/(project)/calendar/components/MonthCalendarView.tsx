...
export default function MonthCalendarView({
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-cu-border bg-cu-bg shadow-cu-sm" data-testid="month-view">
      <div className="grid grid-cols-7 border-b border-cu-border bg-cu-bg-secondary">
        {DAY_NAMES.map((name) => (
        ...
        {days.map((day) => {
          ...
          return (
            <div
              ...
              data-date={key}
              className={[
                'group min-h-[118px] border-b border-r border-cu-border p-2 transition-colors sm:min-h-[142px] sm:p-2.5',
                inMonth ? 'bg-cu-bg' : 'bg-cu-bg-secondary/70 text-cu-text-muted',
                day.getDay() === 0 || day.getDay() === 6 ? 'bg-cu-bg-secondary/50' : '',
                onDayClick ? 'cursor-pointer hover:bg-cu-hover' : '',
                target ? 'bg-cu-primary-light ring-2 ring-inset ring-cu-primary/30' : '',
              ].join(' ')}
